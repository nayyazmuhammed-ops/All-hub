--[[ 
    ALL HUB | KING EDITION
    Authorized: zayan
]]

--// 1. SIMPLE & STABLE LOADING SEQUENCE
local function KingLoading()
    local S = game:GetService("StarterGui")
    local msgs = {"System Booting...", "Bypassing Security...", "Loading All Hub...", "Welcome, Zayan"}
    
    for i, msg in ipairs(msgs) do
        -- This creates the 'Math Hex' look without breaking the script
        local hex = string.format("%X", math.random(100000, 999999))
        print("[" .. hex .. "] " .. msg)
        
        S:SetCore("SendNotification", {
            Title = "🛡️ ALL HUB SECURITY",
            Text = msg,
            Duration = 1
        })
        task.wait(0.2)
